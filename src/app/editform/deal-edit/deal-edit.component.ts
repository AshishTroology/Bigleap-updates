...
export class DealEditComponent implements OnInit {
  ...
  submitForm() {
    ...
    if (this.dealForm.invalid) {
      console.log(this.dealForm.controls);
  ...
  ngOnInit(): void {
    ...
    this.deal
      .getDealData(this._Activatedroute.snapshot.paramMap.get('id'))
      .subscribe((data: any) => {
        console.log(data);
        this.dealData = data[0];
        this.files_url = data[0].attachments;
        // console.log(data[0].product_services)
        this.dealData.approved_by =
          data[0].approved_by && data[0].approved_by != 'undefined'
            ? data[0].approved_by[0]
            : { username: 'NA' };
        this.dealData.verified_by =
          data[0].verified_by && data[0].approved_by != 'undefined'
            ? data[0].verified_by[0]
            : { username: 'NA' };
        ...
        let filter_contact = this.contactData?.filter(
          (cntc: any) => cntc.contact_id === data[0].contact_id
        );
        ...
        let filtered_company = this.accountData?.filter(
          (acc: any) => acc.account_id === data[0].account_id
        );
  ...
  onContactSelect(contact: any) {
    ...
    this.contact_id = filter_contact[0].contact_id;
    this.contact_name = filter_contact[0].contact_name;
    console.log(this.contact_name[0], '---------------->sx');
  ...
  formInit() {
    this.dealForm = this.dlFm.group({
      ...
      company_name: this.dealData?.company_name
        ? this.dealData?.company_name
        : '',
      account_id: this.dealData?.account_id ? this.dealData?.account_id : '',
      contact_name: this.dealData?.contact_name
        ? this.dealData?.contact_name
        : '',
      contact_id: this.contact_id,
      deal_source: [
        this.dealData?.deal_source ? this.dealData?.deal_source : '',
        Validators.required,
      ],
      deal_location: ['', Validators.required],
      deal_title: [
        this.dealData?.deal_title ? this.dealData?.deal_title : '',
        Validators.required,
      ],
      deal_type: [
        this.dealData?.deal_type ? this.dealData?.deal_type : '',
        Validators.required,
      ],
      ...
      deal_estimated_value: this.dealData?.deal_estimated_value
        ? this.dealData?.deal_estimated_value
        : '',
      deal_status_stage: [
        this.dealData?.deal_status_stage
          ? this.dealData?.deal_status_stage
          : '',
        Validators.required,
      ],
      remark: this.dealData?.remark ? this.dealData?.remark : '',
      product_services: this.dealData?.product_services
        ? this.dealData?.product_services
        : [],
      note: this.dealData?.note ? this.dealData?.note : '',
      attachments: '',
      sales_order_no: [
        this.dealData?.sales_order_no ? this.dealData?.sales_order_no : '',
        Validators.required,
      ],
      inco_terms: [this.dealData?.inco_terms ? this.dealData?.inco_terms : ''],
      payment_terms: '',
      p_f: [this.dealData?.p_f ? this.dealData?.p_f : '', Validators.required],
      p_t: [this.dealData?.p_t ? this.dealData?.p_t : '', Validators.required],
      freight: this.dealData?.freight ? this.dealData?.freight : '',
      taxes: this.dealData?.taxes ? this.dealData?.taxes : '',
      other_charges: this.dealData?.other_charges
        ? this.dealData?.other_charges
        : '',
      delivery: this.dealData?.delivery ? this.dealData?.delivery : '',
      warranty: this.dealData?.warranty ? this.dealData?.warranty : '',
      deal_validity: this.dealData?.deal_validity
        ? this.dealData?.deal_validity
        : '',
      created_by: '',
      created_date_time: this.dealData?.created_date_time
        ? this.dealData?.created_date_time
        : '',
      modified_by: '',
      edit_status: true,
  ...
  getTaxableAmount() {
    ...
    this.data.map((item: any) => {
      ...
      console.log(this.taxableAmount, 'dsd');
    });
  }
}
