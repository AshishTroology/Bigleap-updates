...
export class QuoteEditComponent implements OnInit {
  ...
  uom: any;
  logsData: any;
  checkProductGrid: boolean=false;
  ...
  public selectedItemTest: Item = this.listItems[1];
  ...
  ngOnInit(): void {
    ...
    this.quote.getquotelogs(this._Activatedroute.snapshot.paramMap.get("id")).subscribe((data:any)=>{
      console.log(data.result,'logdata')
      this.SetQuoteData(data)
      this.logsData=data.result
      console.log(this.logsData);

    })
    // this.quote.getQuoteById(this._Activatedroute.snapshot.paramMap.get("id")).subscribe
      this.quote.getQuoteById(this.quoteId).subscribe((data:any)=>{

      console.log(data)
      this.quoteData=data.result[0]
        this.quoteOwner = data.result[0].quote_owner;
...
//   }
          ...
          if (item.id === id) {
            ...
            if(v!=null){
              if(v.products.length==0){
                ...
                v.UnitPrice = this.calculateAmount(v.products);
                console.log(v,"============================>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>.")
              }
              this.data=this.data.filter((dt:any)=>dt)
            }
  ...
  calculateAmount(arr:any){
    ...
    if(arr.length==0){
      ...
      arr.map((item:any)=>{
        amtt+=parseFloat(item.UnitPrice);
      })
      return amtt;
    }
  }
  ...
  formInit(){
    this.quoteForm = this.qtFm.group({
      ...
      quote_owner: [this.quoteOwner, Validators.required],
      account_id: this.account_id,
      contact_id: this.contact_id,
      quote_title: [
        this.quoteData?.quote_title ? this.quoteData?.quote_title : '',
        Validators.required,
      ],
      quote_type: [
        this.quoteData?.quote_type ? this.quoteData?.quote_type : '',
        Validators.required,
      ],
      quote_source: [
        this.quoteData?.quote_source ? this.quoteData?.quote_source : '',
        Validators.required,
      ],
      ...
      estimated_value: this.quoteData?.estimated_value
        ? this.quoteData?.estimated_value
        : '',
      quote_status: [
        this.quoteData?.quote_status ? this.quoteData?.quote_status : '',
        Validators.required,
      ],
      quote_location: ['', Validators.required],
      remark: this.quoteData?.remark ? this.quoteData?.remark : '',
      product_services: this.quoteData?.product_services
        ? this.quoteData?.product_services
        : '',
      note: this.quoteData?.note ? this.quoteData?.note : '',
      attachments: '',
      open_activity: this.quoteData?.open_activity
        ? this.quoteData?.open_activity
        : '',
      close_activity: this.quoteData?.close_activity
        ? this.quoteData?.close_activity
        : '',
      inco_terms: [
        this.quoteData?.inco_terms ? this.quoteData?.inco_terms : '',
      ],
      payment_terms: '',

      p_f: [
        this.quoteData?.p_f ? this.quoteData?.p_f : '',
        Validators.required,
      ],
      p_t: [
        this.quoteData?.p_t ? this.quoteData?.p_t : '',
        Validators.required,
      ],
      freight: [
        this.quoteData?.freight ? this.quoteData?.freight : '',
        Validators.required,
      ],
      taxes: [
        this.quoteData?.taxes ? this.quoteData?.taxes : '',
        Validators.required,
      ],
      other_charges: this.quoteData?.other_charges
        ? this.quoteData?.other_charges
        : '',
      deal_validity: this.quoteData?.deal_validity
        ? this.quoteData?.deal_validity
        : '',
      delivery: [
        this.quoteData?.delivery ? this.quoteData?.delivery : '',
        Validators.required,
      ],
      warrenty: [
        this.quoteData?.warrenty ? this.quoteData?.warrenty : '',
        Validators.required,
      ],
      created_by: this.quoteData?.created_by ? this.quoteData?.created_by : '',
      created_date_time: this.quoteData?.created_date_time
        ? this.quoteData?.created_date_time
        : '',
...
handleHistoryView(id:any){
  console.log(id)
  this.quote.getOneLog(id).subscribe((data:any)=>{
    console.log(data)
    this.SetQuoteData(data)
    // this.forminit(data)
  })
